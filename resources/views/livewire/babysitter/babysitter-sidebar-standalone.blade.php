...
<div class="w-64 bg-white shadow-lg h-screen fixed left-0 top-0 z-40">
        ...
        <div class="flex items-center space-x-3">
            ...
            @else
                <div class="w-12 h-12 bg-gray-300 rounded-full flex items-center justify-center">
                    <span class="text-gray-600 font-bold">{{ substr($babysitter?->utilisateur?->prenom ?? 'B', 0, 1) }}</span>
                </div>
            @endif
            <div class="flex-1">
                ...
                <div class="flex items-center space-x-1">
                    ...
                    <span class="text-sm text-gray-600">{{ number_format($babysitter?->utilisateur?->note ?? 0, 1) }}</span>
                </div>
            </div>
        </div>
    </div>
                ...
                <a href="#" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-gray-700 hover:bg-gray-100 transition-colors">
                    ...
                    <span>Notifications
                        @if($pendingRequestsCount > 0)
                            <span class="ml-2 bg-red-500 text-white text-xs px-2 py-1 rounded-full">{{ $pendingRequestsCount }}</span>
                        @endif
                    </span>
                </a>
            </li>
    ...
    <div class="absolute bottom-0 left-0 right-0 p-6 border-t border-gray-200">
        <a href="/logout" class="flex items-center space-x-3 px-4 py-3 rounded-lg text-red-600 hover:bg-red-50 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
